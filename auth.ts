...
type Tab = "recs" | "profile" | "settings" | "chats" | "admin";
type RegMode = "user" | "admin" | null;
...
function App() {
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-md flex-col">
      ...
      {/* Регистрация при первом входе: выбор типа */}
        ...
        <main className="fade-up px-4 pb-16 pt-10">
          <div className="mb-8 flex flex-col items-center text-center">
            ...
            <p className="mt-1 max-w-xs text-sm" style={{ color: "var(--muted)" }}>
              Привет, {me.firstName || "игрок"}! Выбери тип регистрации — и
              вперёд за тиммейтами.
            </p>
          </div>
          ...
          <div className="space-y-3">
            <button
              ...
              className="btn-cut w-full rounded-2xl bg-gradient-to-br from-red-500 to-orange-500 p-4 text-left shadow-xl shadow-red-500/25 transition-transform active:scale-[0.98]"
            >
              <span className="block text-lg font-black text-white">
                👥 Для игроков
              </span>
              <span className="mt-0.5 block text-xs text-white/85">
                Создать анкету и искать тиммейтов в Dota 2
              </span>
            </button>
            ...
            <button
              ...
              className="w-full rounded-2xl border p-4 text-left transition-transform active:scale-[0.98]"
              style={{
                borderColor: "color-mix(in srgb, var(--accent) 45%, transparent)",
                background: "var(--surface)",
              }}
            >
              <span className="block text-lg font-black" style={{ color: "var(--text)" }}>
                🛡️ Для админов
              </span>
              <span className="mt-0.5 block text-xs" style={{ color: "var(--muted)" }}>
                Требуется код доступа · откроется админская панель
              </span>
            </button>
          </div>
      ...
      {/* Регистрация при первом входе: форма */}
      {stage === "onboarding" && me && regMode !== null && (
        <main className="fade-up px-4 pb-16 pt-6">
          <div className="mb-5 flex flex-col items-center text-center">
            ...
            <h1
              ...
              style={{ color: "var(--text)" }}
            >
              {regMode === "admin" ? "Регистрация админа" : "Создай анкету"}
            </h1>
            <p className="mt-1 max-w-xs text-sm" style={{ color: "var(--muted)" }}>
              {regMode === "admin"
                ? "Заполни анкету и введи код доступа — откроется админская панель."
      ...
      {stage === "app" && me && (
        <main className="flex min-h-[100dvh] flex-col">
          {tab === "recs" && <RecommendationsView me={me} />}
          {tab === "profile" && (
          ...
          {tab === "chats" && <ChatsView onGoRecs={() => setTab("recs")} />}
          {tab === "admin" && me.isAdmin && <AdminPanelView />}
        </main>
      )}
