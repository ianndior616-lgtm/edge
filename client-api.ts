...
import type { AdminUsersResponse, AdminUserView } from "@/lib/types";
...
export function AdminPanelView() {
  ...
  const load = async (query: string) => {
    setLoading(true);
    try {
      const data = await api<AdminUsersResponse>(
  ...
  /** Ручная выдача арканы (веха 30 рефералов) */
              ...
              >
                ...
                {expanded && !editing && (
                  <div className="mt-3 space-y-2 border-t pt-3" style={{ borderColor: "var(--border)" }}>
                    ...
                    {/* Аркана за 30 рефералов — выдача вручную */}
                    {u.arcanaIssued ? (
                      <div
