...
export function OnboardingForm({
  ...
  /** Режим регистрации администратора — требует код доступа */
  adminMode?: boolean;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) return setError("Укажи своё имя");
    ...
    // Админ-регистрация: проверяем код доступа перед сохранением
    if (adminMode) {
      if (!adminCode.trim()) return setError("Введи код доступа администратора");
      setSaving(true);
      try {
        const verify = await api<{ ok: boolean }>("/api/admin/verify", initData, {
          method: "POST",
          body: { code: adminCode.trim() },
        });
        if (!verify.ok) {
          setSaving(false);
          return setError("Неверный код доступа. Попробуй ещё раз.");
        }
      } catch {
        setSaving(false);
        return setError("Не удалось проверить код — проверь соединение");
      }
    }
    ...
    const baseBody = {
      ...
      ...(adminMode ? { adminCode: adminCode.trim() } : {}),
    };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {adminMode && (
        ...
        >
          <label className={LABEL_CLS} htmlFor="pf-code">
            🔑 Код доступа администратора
          </label>
          <input
            id="pf-code"
          ...
          <p className="mt-1.5 text-[11px]" style={{ color: "var(--dim)", opacity: 0.85 }}>
            Код выдаётся администрацией EdGGe. Без верного кода админ-панель
            не откроется.
          </p>
        </div>
      )}
        ...
        >
          {saving
            ...
            : initial
              ? "💾 Сохранить анкету"
              : adminMode
                ? "🛡️ Зарегистрироваться как админ"
