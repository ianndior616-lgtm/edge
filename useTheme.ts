...
export function ProfileView({
  ...
  const nextReward = rewardForStreak(me.streakDays + 1);
          ...
          >
            ...
            <div className="mt-3 grid grid-cols-7 gap-1.5">
              {DAILY_REWARDS.map((r, i) => {
                const day = i + 1;
                const done = me.streakDays >= day;
                const isToday = !claimedToday && me.streakDays + 1 === day;
                return (
            ...
            <div className="mt-3">
              ...
              <div className="mt-1.5 h-2 w-full overflow-hidden rounded-full" style={{ background: "var(--surface2)" }}>
                <div
                  ...
                  style={{ width: `${Math.min(100, (me.referralCount / 30) * 100)}%` }}
                />
              </div>
              ...
              <div className="mt-3 grid grid-cols-4 gap-1.5">
                {REFERRAL_MILESTONES.map((m) => {
                  const reached = me.referralCount >= m.count;
                  return (
                    <div
                      key={m.count}
